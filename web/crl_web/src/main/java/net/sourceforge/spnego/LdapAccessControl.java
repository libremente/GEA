...
/**
 * The <code>LdapAccessControl</code> class is a reference implementation of the
 * {@link UserAccessControl} interface. This class only performs user
 * authorization (authZ) and not user authentication (authN). This class
 * implements an authZ model in a similar style as the
 ...
 * Based Access Control</a> (ABAC) model. However, this pedagogical
 * implementation is much simpler and limited than the two formal models.
 *
 * <p>
 ...
 *
 * <p>
 * The <code>LdapAccessControl</code> implementation makes calls to an LDAP
 * server (e.g. Microsoft's Active Directory (AD) server) when performing a
 * lookup to determine if a user has one or more attributes defined. The LDAP
 * queries are based on LDAP search/fiter criteria(s) defined at the time an
 * instance of this class is placed into service. An instance of this class is
 * considered to be in service after invoking the <code>init</code> method.
 * </p>
 *
 * <p>
 * In the SPNEGO Library, the <code>SpnegoHttpFilter</code> class is the
 * mechanism that performs user authentication (authN) whilst the
 * <code>LdapAccessControl</code> class is the default mechanism that performs
 * user authorization (authZ). This default can be replaced by any class that
 * implement the <code>UserAccessControl</code> interface. To change the
 * default, specify the new class in the SPNEGO Library's filter definition
 * section of the web.xml file.
 * </p>
 *
 * <p>
 * Authorization (authZ) is an optional feature of the SPNEGO Library. The
 * SPNEGO Library provides an interface, {@link SpnegoAccessControl}, to
 * applications that need authZ capability. Applications can check a user's
 * authZ by calling methods defined in the <code>SpnegoAccessControl</code>
 * interface.
 * </p>
 *
 * <p>
 * The <code>LdapAccessControl</code> class is configured within the same
 * web.xml filter section as the <code>SpnegoHttpFilter</code> class. The
 * configuration is specified by adding additional filter parameters to the
 * <code>SpnegoHttpFilter</code> filter definition.
 * </p>
 *
 * <p>
 ...
 *         &lt;param-value&gt;net.sourceforge.LdapAccessControl&lt;/param-value&gt;
 *     &lt;/init-param&gt;
 *     &lt;init-param&gt;
 ...
 *
 * <p>
 * As an alternative option, the <code>spnego.authz.ldap.filter.[i]</code>
 * parameters and the <code>spnego.authz.resource.[name|access|type].[i]</code>
 * parameters may be specified in a policy file.
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * For more information on how a web application/service can leverage access
 * controls or to view some usage examples, please read the javadoc of the
 * {@link SpnegoAccessControl} interface and the javadoc of the
 ...
 * "_blank">authZ for standalone apps</a> example and the
...
public class LdapAccessControl implements UserAccessControl {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.sourceforge.UserAccessControl#anyRole(java.lang.String,
	 * java.lang.String[])
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.sourceforge.UserAccessControl#hasRole(java.lang.String,
	 * java.lang.String)
	 */
	@Override
		...
		try {
			...
			try {
				...
				for (String filter : this.policy) {
					...
					final NamingEnumeration<SearchResult> results = context.search(this.deecee,
							String.format(filter, username, attribute), this.srchCntrls);
					...
					if (count > 1 && this.uniqueOnly) {
						...
						throw new IllegalArgumentException(
								"Uniqueness property violated. " + "Found duplicate role/attribute:" + attribute
										+ ". This MAY be caused by an improper policy definition" + "; filter=" + filter
										+ "; policy=" + this.policy);
					}
				}
				context.close();
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.sourceforge.UserAccessControl#anyAccess(java.lang.String,
	 * java.lang.String[])
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.sourceforge.UserAccessControl#hasAccess(java.lang.String,
	 * java.lang.String)
	 */
	@Override
	...
	public void init(final Properties props) {
		...
		env.put(Context.INITIAL_CONTEXT_FACTORY, policies.getProperty(LDAP_FACTORY,
				props.getProperty(LDAP_FACTORY, "com.sun.jndi.ldap.LdapCtxFactory")));
		env.put(Context.SECURITY_AUTHENTICATION,
				policies.getProperty(LDAP_AUTHN, props.getProperty(LDAP_AUTHN, "Simple")));
		env.put("com.sun.jndi.ldap.connect.pool",
				policies.getProperty(LDAP_POOL, props.getProperty(LDAP_POOL, "true")));

		String dc = policies.getProperty(LDAP_DEECE, props.getProperty(LDAP_DEECE, ""));
		if (dc.isEmpty()) {
			final String tmp = props.getProperty(SERVER_REALM, policies.getProperty(SERVER_REALM, ""));
			if (tmp.trim().isEmpty()) {
				throw new IllegalArgumentException(
						"MUST provide the serve's deecee. " + " specify a value for the " + LDAP_DEECE + " property.");
			}
		...
		if (policies.getProperty(LDAP_URL, props.getProperty(LDAP_URL, "")).isEmpty()) {
			...
			env.put(Context.PROVIDER_URL, policies.getProperty(LDAP_URL, props.getProperty(LDAP_URL)));
		...
		env.put(Context.SECURITY_PRINCIPAL, policies.getProperty(LDAP_USERNAME, props.getProperty(LDAP_USERNAME,
				props.getProperty(KRB5_USERNAME, policies.getProperty(KRB5_USERNAME)))));
		env.put(Context.SECURITY_CREDENTIALS, policies.getProperty(LDAP_PASSWORD, props.getProperty(LDAP_PASSWORD,
		...
		try {
			...
			final String[] labels = policies.getProperty(USER_INFO, props.getProperty(USER_INFO, "")).split(",");
	...
	private void loadPolicies(final Properties props, final Properties policies) {
		for (int i = 0; i <= MAX_NUM_FILTERS; i++) {
			final int idx = i + 1;
			final String filter = policies.getProperty(PREFIX_FILTER + idx, props.getProperty(PREFIX_FILTER + idx, ""))
					.trim();
	...
	private void loadResourceNames(final Properties props, final Properties policies) {
		...
		for (int i = 0; i <= MAX_NUM_FILTERS; i++) {
			...
			final String resname = policies.getProperty(PREFIX_NAME + idx, props.getProperty(PREFIX_NAME + idx, ""))
					.trim();

			final String restype = policies.getProperty(PREFIX_TYPE + idx,
					props.getProperty(PREFIX_TYPE + idx, "").toLowerCase().trim());

			final String[] resaccess = policies
					.getProperty(PREFIX_ACCESS + idx, props.getProperty(PREFIX_ACCESS + idx, "")).trim().split(",");
	...
	private UserInfo cacheUserInfo(final String username) throws NamingException {
		...
		final NamingEnumeration<SearchResult> results = context.search(this.deecee,
				String.format(this.userInfoFilter, username), this.srchCntrls);
		...
		if (found) {
			...
			userInfoObject = new UserInfo() {
				...
				public List<String> getInfo(final String label) {
					if (!hasInfo(label)) {
						throw new NullPointerException("UserInfo label not found or not in user store: " + label
			...
			throw new IllegalArgumentException("UserInfo not found. "
					+ ". This MAY be caused by an incorrect spnego.authz.ldap.user.filter definition" + "; filter="
					+ this.userInfoFilter + "; policy=" + this.policy);
		}
